...
with T[0]:
    ...
    colors = ['#4f8ef7', '#38d9a9', '#f59f00', '#9775fa', '#f03e3e', '#40c057', '#ffd43b', '#74c0fc', '#ff8787', '#cc5de8']
...
with T[6]:
    ...
    fig.add_trace(go.Scatter(x=merged['date'], y=merged['recibidos'], mode='lines+markers',
                             name='Recibidos', line=dict(color='#4f8ef7', width=2)))
    fig.add_trace(go.Scatter(x=merged['date'], y=merged['resueltos'], mode='lines+markers',
                             name='Resueltos', line=dict(color='#40c057', width=2)))
    ...
    with ac1:
        ...
        fig = px.bar(merged, x='date', y='recibidos', color_discrete_sequence=['#4f8ef7'])
        apply_theme(fig, height=260)
    ...
    with ac2:
        ...
        fig = px.bar(merged, x='date', y='resueltos', color_discrete_sequence=['#40c057'])
        apply_theme(fig, height=260)
        fig.update_layout(showlegend=False)
        st.plotly_chart(fig, use_container_width=True)
    ...
    with ec1:
        fig = px.bar(estado_counts, x='estado', y='count', color='estado',
                     ...
                     color_discrete_map={'Open': '#f03e3e', 'Pending': '#f59f00',
                                         'Resolved': '#40c057', 'Closed': '#94a3b8'})
        apply_theme(fig, height=300)
        fig.update_layout(showlegend=False)
        st.plotly_chart(fig, use_container_width=True)
